...
export default function Header() {
  ...
  return (
    <>
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'glass shadow-soft border-b border-canvas-200/50' : 'bg-transparent'}`}>
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className={`flex items-center justify-between transition-all duration-300 ${scrolled ? 'h-14' : 'h-16 lg:h-18'}`}>
            ...
            <a href="#hem" className="flex items-center gap-2.5 group">
              <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-accent-600 transition-transform duration-300 group-hover:scale-105">
            ...
            <nav className="hidden lg:flex items-center gap-1">
              {NAV_LINKS.map((link) => (
                <a key={link.href} href={link.href}
                  ...
                  {link.label}
                  {active === link.href.slice(1) && (
                    <span className="absolute bottom-1 left-1/2 -translate-x-1/2 h-0.5 w-5 rounded-full bg-accent-600 transition-all duration-300" />
                  )}
                </a>
              ))}
            </nav>
            ...
            <div className="flex items-center gap-2.5">
              <a href={COMPANY.phoneHref}
                className="shimmer flex items-center gap-2 rounded-xl bg-accent-600 px-4 lg:px-5 py-2.5 text-sm font-semibold text-white shadow-accent transition-all duration-300 hover:bg-accent-700 hover:-translate-y-0.5">
                <Phone className="h-4 w-4" strokeWidth={2.5} />
                <span className="hidden sm:inline">Ring oss</span>
                <span className="sm:hidden tabular-nums tracking-tight">{COMPANY.phoneDisplay}</span>
              </a>
              <button onClick={() => setMobileOpen(true)}
                className={`lg:hidden flex h-10 w-10 items-center justify-center rounded-lg transition-colors ${scrolled ? 'text-navy-800' : 'text-white'}`}
        ...
        <div className={`absolute right-0 top-0 h-full w-72 max-w-[85vw] bg-white shadow-2xl transition-transform duration-300 ${mobileOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          ...
          <div className="mt-auto p-6 border-t border-canvas-200">
            <a href={COMPANY.phoneHref}
              ...
              <Phone className="h-4 w-4" strokeWidth={2.5} />
              Ring oss – <span className="tabular-nums tracking-tight">{COMPANY.phoneDisplay}</span>
            </a>
          </div>
        </div>
      </div>
